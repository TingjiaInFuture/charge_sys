...
class UserClient(tk.Tk):
    ...
    def show_charging_details(self):
        ...
        # 充电详单框架
        details_frame = ttk.LabelFrame(self.main_frame, text="充电详单", padding=20)
        ...
        try:
            # 获取充电详单
            details = self.network_client.get_charging_details(self.car_id)
            if details:
                # 显示当前充电请求
                current_request = details.get('current_request')
                if current_request:
                    self.details_tree.insert("", "end", values=(
                        current_request['status'],
                        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        "等待分配",
                        f"{current_request['request_amount_kwh']:.2f}",
                        "等待中",
                ...
                # 显示当前充电会话
                current_session = details.get('current_session')
                if current_session:
                    self.details_tree.insert("", "end", values=(
                        "进行中",
                        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    ...
                    # 添加结束充电按钮
                    end_charging_frame = ttk.Frame(self.main_frame)
                    end_charging_frame.pack(fill=tk.X, pady=10)
                    
                    ttk.Button(
                        end_charging_frame,
                        text="结束充电",
                        command=lambda: self.end_charging(current_session)
                    ).pack(side=tk.LEFT, padx=5)
                
                # 显示历史账单
                for bill in details.get('bills', []):
            ...
            if not details or (not details.get('current_request') and not details.get('current_session') and not details.get('bills')):
                ttk.Label(details_frame, text="暂无充电记录").pack(pady=20)
                
        except Exception as e:
            messagebox.showerror("错误", f"获取充电详单失败: {str(e)}")
        
        # 返回按钮
        ttk.Button(self.main_frame, text="返回主菜单", command=self.show_main_menu).pack(pady=10)
    
    def end_charging(self, session):
        """结束充电"""
        try:
            # 确认是否结束充电
            if not messagebox.askyesno("确认", "确定要结束充电吗？"):
                return
