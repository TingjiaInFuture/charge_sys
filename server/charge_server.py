...
class ChargeServer:
    ...
    def _handle_end_charging(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self.charging_service.end_charging(car_id)
            return {'status': 'success', 'message': '充电已结束'}
        except Exception as e:
            return {'status': 'error', 'message': str(e)}
    ...
    def _handle_get_charging_details(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for request in self.request_repo.get_all():
                if request.car_id == car_id and request.state in [CarState.WAITING_IN_MAIN_QUEUE, CarState.WAITING_AT_PILE_QUEUE]:
            ...
            # 获取历史账单
            bills = []
            for bill in self.bill_repo.get_all():
                if bill.car_id == car_id:
                    bills.append({
                        'bill_id': bill.bill_id,
                        'pile_id': bill.pile_id,
                        'start_time': bill.start_time.isoformat(),
                        'end_time': bill.end_time.isoformat(),
